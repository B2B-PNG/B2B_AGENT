...
const RestaurantDetail = () => {
  ...
  const { rdlData, rdLoading, rdError } = useDetailRestaurant(filters);
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="flex-1">
          ...
          <div className="flex text-orange-400 mt-2">
            {[1, 2, 3, 4, 5].map((star) => (
              <Star
                key={star}
                className={`w-4 h-4 ${
                  star <= (restaurant?.intEasiaCateID ?? 0) ? "fill-current" : ""
                }`}
              />
            ))}
          </div>
          ...
          <div className="mt-4 space-y-3 text-sm">
            ...
            <div className="">
            <p className="bold text-500">
              Mô tả: 
            </p>
            <span className="italic text-gray-500"> {restaurant?.strDescription || "Chưa có mô tả"}</span>
            </div>
          </div>
        </div>
      </div>
